...
def finder(subreddit):
    ...
    content = tuple(zip(post_names,sub_link_list)) #tuple of title and sublink for video file creation
    return content
...
def get_vids(sub_folder, content):
    ...
    for title, link in tqdm(content): #go through tuple each sublink, download and save to our new directory under reddit title
        try:
            ...
            url = (f'https://api.streamable.com/videos/{link}')
            user = (creds.streamable_user) 
            password = (creds.streamable_pw)
            response = request.get(url, auth=(user, password), stream = True) #make initial request using headers
            url_str = response.json()['files']['mp4']['url']
            url_str = ('https:'+ url_str)
            r = request.get(url_str, auth=(user, password), stream = True) 
